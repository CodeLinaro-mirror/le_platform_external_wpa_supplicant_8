...
int wpa_set_wep_keys(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid) {
  ...
  for (i = 0; i < NUM_WEP_KEYS; i++) {
    if (ssid->wep_key_len[i] == 0) continue;

    set = 1;
    wpa_drv_set_key(wpa_s, -1, WPA_ALG_WEP, NULL, i, i == ssid->wep_tx_keyidx,
                    NULL, 0, ssid->wep_key[i], ssid->wep_key_len[i],
                    i == ssid->wep_tx_keyidx ? KEY_FLAG_GROUP_RX_TX_DEFAULT
...
void wpas_auth_timeout_restart(struct wpa_supplicant *wpa_s, int sec_diff) {
  ...
  if (eloop_is_timeout_registered(wpa_supplicant_timeout, wpa_s, NULL)) {
    wpa_dbg(wpa_s, MSG_DEBUG, "Authentication timeout restart: %d sec",
            new_sec);
    eloop_cancel_timeout(wpa_supplicant_timeout, wpa_s, NULL);
...
void wpa_supplicant_initiate_eapol(struct wpa_supplicant *wpa_s) {
  ...
  if (wpa_s->key_mgmt == WPA_KEY_MGMT_IEEE8021X_NO_WPA) {
    ...
    if (wpa_s->drv_flags & WPA_DRIVER_FLAGS_WIRED) eapol_conf.required_keys = 0;
  }
  ...
  if (wpa_s->key_mgmt == WPA_KEY_MGMT_WPS) {
    ...
    if (wpa_s->current_bss) {
      ...
      if (ie) {
        if (wps_is_20(ie)) eapol_conf.wps |= EAPOL_PEER_IS_WPS20_AP;
        wpabuf_free(ie);
      }
    }
  }
...
static void remove_bss_tmp_disallowed_entry(
    struct wpa_supplicant *wpa_s, struct wpa_bss_tmp_disallowed *bss) {
...
static struct wpabuf *wpas_wfa_gen_capab_attr(struct wpa_supplicant *wpa_s) {
  ...
  if (wpa_s->conf->wfa_gen_capa == WFA_GEN_CAPA_DISABLED) return NULL;

  if (!wpa_s->conf->wfa_gen_capa_supp ||
      wpabuf_len(wpa_s->conf->wfa_gen_capa_supp) == 0) {
    supp_len = 1;
    supp_buf[0] = 0;
    if (wpa_s->hw_capab & BIT(CAPAB_HT)) supp_buf[0] |= BIT(0);  /* Wi-Fi 4 */
    if (wpa_s->hw_capab & BIT(CAPAB_VHT)) supp_buf[0] |= BIT(1); /* Wi-Fi 5 */
    if (wpa_s->hw_capab & BIT(CAPAB_HE)) supp_buf[0] |= BIT(2);  /* Wi-Fi 6 */
    if (wpa_s->hw_capab & BIT(CAPAB_EHT)) supp_buf[0] |= BIT(3); /* Wi-Fi 7 */
    supp = supp_buf;
  } else {
  ...
  if (add_cert) {
    wpabuf_put_u8(attr, wpabuf_len(wpa_s->conf->wfa_gen_capa_cert));
...
static void wpas_wfa_capab_tx(void *eloop_ctx, void *timeout_ctx) {
  ...
  if (wpa_s->conf->wfa_gen_capa != WFA_GEN_CAPA_PROTECTED ||
      wpa_s->wpa_state != WPA_COMPLETED || !pmf_in_use(wpa_s, wpa_s->bssid))
    return;
  ...
  if (wpa_drv_send_action(wpa_s, wpa_s->assoc_freq, 0, wpa_s->bssid,
                          ...
                          wpabuf_len(buf), 0) < 0)
    wpa_printf(MSG_DEBUG, "WFA: Failed to send WFA Capabilities frame");
...
static void wpa_supplicant_cleanup(struct wpa_supplicant *wpa_s) {
   ...
   */
  if (wpa_s->radio) radio_remove_works(wpa_s, "gas-query", 0);
...
#ifdef CONFIG_MESH
  {
    ...
    while ((entry = dl_list_last(&wpa_s->mesh_external_pmksa_cache,
                                 struct external_pmksa_cache, list)) != NULL) {
...
void wpa_clear_keys(struct wpa_supplicant *wpa_s, const u8 *addr) {
  ...
  for (i = 0; i < max; i++) {
    if (wpa_s->keys_cleared & BIT(i)) continue;
    wpa_drv_set_key(wpa_s, -1, WPA_ALG_NONE, NULL, i, 0, NULL, 0, NULL, 0,
                    KEY_FLAG_GROUP);
  }
  ...
  if (~wpa_s->keys_cleared & (BIT(0) | BIT(15)) && addr &&
    ...
    if (!(wpa_s->keys_cleared & BIT(0)))
      wpa_drv_set_key(wpa_s, -1, WPA_ALG_NONE, addr, 0, 0, NULL, 0, NULL, 0,
                      KEY_FLAG_PAIRWISE);
    if (!(wpa_s->keys_cleared & BIT(15)))
      wpa_drv_set_key(wpa_s, -1, WPA_ALG_NONE, addr, 1, 0, NULL, 0, NULL, 0,
                      KEY_FLAG_PAIRWISE);
...
void wpa_supplicant_reset_bgscan(struct wpa_supplicant *wpa_s) {
  ...
  if (wpas_driver_bss_selection(wpa_s)) return;
#ifdef CONFIG_P2P
  if (wpa_s->p2p_group_interface != NOT_P2P_GROUP_INTERFACE) return;
#endif /* CONFIG_P2P */
...
static void wpas_verify_ssid_beacon(void *eloop_ctx, void *timeout_ctx) {
  ...
  if (!wpa_s->current_ssid || !wpa_s->current_bss) return;
  ...
  if (wpa_s->wpa_state < WPA_4WAY_HANDSHAKE || !wpa_s->bigtk_set ||
      wpa_s->ssid_verified)
    return;

  wpa_printf(MSG_DEBUG,
             "SSID not yet verified; check if the driver has received a "
             "verified Beacon frame");
  if (wpa_supplicant_update_scan_results(wpa_s, wpa_s->bssid) < 0) return;
  ...
  if (bss->tsf > wpa_s->first_beacon_tsf) {
    ...
    wpa_printf(MSG_DEBUG, "Verified Beacon frame has been received");
    wpa_s->beacons_checked++;

    ie = wpa_bss_get_ie_beacon(bss, WLAN_EID_SSID);
    if (ie && ie[1] == ssid_len && os_memcmp(&ie[2], ssid, ssid_len) == 0) {
      wpa_printf(MSG_DEBUG,
  ...
  if (wpa_s->beacons_checked < 16) {
    eloop_register_timeout(wpa_s->next_beacon_check, 0, wpas_verify_ssid_beacon,
                           wpa_s, NULL);
...
void wpa_supplicant_set_state(struct wpa_supplicant *wpa_s,
  ...
  if (state != WPA_SCANNING) wpa_supplicant_notify_scanning(wpa_s, 0);
  ...
  if (state == WPA_COMPLETED && wpa_s->new_connection) {
    ...
    if (wpa_s->valid_links)
      os_snprintf(mld_addr, sizeof(mld_addr), " ap_mld_addr=" MACSTR,
    ...
    if (wpa_s->conf->wfa_gen_capa == WFA_GEN_CAPA_PROTECTED &&
      ...
      eloop_cancel_timeout(wpas_wfa_capab_tx, wpa_s, NULL);
      eloop_register_timeout(0, 100000, wpas_wfa_capab_tx, wpa_s, NULL);
    }
  ...
  if (wpa_s->wpa_state != old_state) {
    ...
    if (wpa_s->wpa_state == WPA_COMPLETED || old_state == WPA_COMPLETED)
      wpas_notify_auth_changed(wpa_s);
#ifdef CONFIG_DPP2
    if (wpa_s->wpa_state == WPA_COMPLETED) wpas_dpp_connected(wpa_s);
#endif /* CONFIG_DPP2 */

    if (wpa_s->wpa_state == WPA_COMPLETED && wpa_s->bigtk_set &&
        !wpa_s->ssid_verified)
      wpas_verify_ssid_beacon_prot(wpa_s);
  }
#if defined(CONFIG_FILS) && defined(IEEE8021X_EAPOL)
  if (update_fils_connect_params) wpas_update_fils_connect_params(wpa_s);
#endif /* CONFIG_FILS && IEEE8021X_EAPOL */
}
...
void wpa_supplicant_terminate_proc(struct wpa_global *global) {
  ...
  while (wpa_s) {
    ...
    if (wpas_wps_terminate_pending(wpa_s) == 1) pending = 1;
#ifdef CONFIG_P2P
...
int wpa_supplicant_reload_configuration(struct wpa_supplicant *wpa_s) {
  ...
  if (wpa_s->confname == NULL) return -1;
  ...
  if (wpa_s->current_ssid) {
    if (wpa_s->wpa_state >= WPA_AUTHENTICATING) wpa_s->own_disconnect_req = 1;
    wpa_supplicant_deauthenticate(wpa_s, WLAN_REASON_DEAUTH_LEAVING);
  }
  ...
  wpa_s->conf = conf;
  if (old_ap_scan != wpa_s->conf->ap_scan) wpas_notify_ap_scan_changed(wpa_s);

  if (reconf_ctrl) wpa_s->ctrl_iface = wpa_supplicant_ctrl_iface_init(wpa_s);
...
static int matching_ciphers(struct wpa_ssid *ssid, struct wpa_ie_data *ie,
                            int freq) {
  if (!ie->has_group) ie->group_cipher = wpa_default_rsn_cipher(freq);
  if (!ie->has_pairwise) ie->pairwise_cipher = wpa_default_rsn_cipher(freq);
...
void wpas_set_mgmt_group_cipher(struct wpa_supplicant *wpa_s,
  ...
  if (ssid->group_mgmt_cipher) sel &= ssid->group_mgmt_cipher;
  if (wpas_get_ssid_pmf(wpa_s, ssid) == NO_MGMT_FRAME_PROTECTION ||
      !(ie->capabilities & WPA_CAPABILITY_MFPC))
    sel = 0;
  wpa_dbg(wpa_s, MSG_DEBUG,
          "WPA: AP mgmt_group_cipher 0x%x network profile mgmt_group_cipher "
          "0x%x; available mgmt_group_cipher 0x%x",
...
static int wpa_supplicant_get_psk(struct wpa_supplicant *wpa_s,
  ...
  if (bss && ssid->bssid_set && ssid->ssid_len == 0 && ssid->passphrase) {
    if (pbkdf2_sha1(ssid->passphrase, bss->ssid, bss->ssid_len, 4096, psk,
                    PMK_LEN) != 0) {
  ...
  if (ssid->ext_psk) {
    ...
    if (wpabuf_len(pw) >= 8 && wpabuf_len(pw) < 64 && bss) {
      if (pbkdf2_sha1(pw_str, bss->ssid, bss->ssid_len, 4096, psk, PMK_LEN) !=
          0) {
        wpa_msg(wpa_s, MSG_WARNING, "Error in pbkdf2_sha1()");
      ...
      wpa_hexdump_key(MSG_MSGDUMP, "PSK (from external passphrase)", psk,
                      PMK_LEN);
    } else
#endif /* CONFIG_NO_PBKDF2 */
...
#ifdef CONFIG_SAE_PK
  if (ssid->sae_pk) capab |= BIT(WLAN_RSNX_CAPAB_SAE_PK);
#endif /* CONFIG_SAE_PK */
#endif /* CONFIG_SAE */
...
int wpa_supplicant_set_suites(struct wpa_supplicant *wpa_s, struct wpa_bss *bss,
                              ...
                              size_t *wpa_ie_len, bool skip_default_rsne) {
  struct wpa_ie_data ie;
  int sel, proto;
#ifdef CONFIG_SAE
  enum sae_pwe sae_pwe;
#endif /* CONFIG_SAE */
  const u8 *bss_wpa, *bss_rsn, *bss_rsnx, *adaptive_11r_ie;
  bool wmm;
  ...
  if (bss || !wpa_s->ap_ies_from_associnfo) {
    ...
    if (bss) {
      ...
      rsnoe = wpa_bss_get_vendor_ie(bss, RSNE_OVERRIDE_IE_VENDOR_TYPE);
      rsno2e = wpa_bss_get_vendor_ie(bss, RSNE_OVERRIDE_2_IE_VENDOR_TYPE);
      rsnxoe = wpa_bss_get_vendor_ie(bss, RSNXE_OVERRIDE_IE_VENDOR_TYPE);
    }
  ...
  wpa_dbg(wpa_s, MSG_DEBUG,
          "WPA: AP pairwise 0x%x network profile pairwise 0x%x; available "
          "pairwise 0x%x",
  ...
  if (bss && (wpa_s->drv_flags2 & WPA_DRIVER_FLAGS_ADAPTIVE_11R) &&
    ...
    if (!adaptive_11r_ie) {
      wpa_printf(MSG_INFO, "Check for the Adaptive_11r_ie's from beacon ie's");
      adaptive_11r_ie =
          wpa_bss_get_vendor_ie_beacon(bss, ADAPTIVE_11R_IE_VENDOR_TYPE);
    }

    if (adaptive_11r_ie && (adaptive_11r_ie[6] & 0x1)) {
      wpa_msg(wpa_s, MSG_ERROR, "Adaptive 11r is Enabled for BSS " MACSTR "",
              MAC2STR(bss->bssid));
      sel = (sel & WPA_KEY_MGMT_PSK) ? WPA_KEY_MGMT_FT_PSK
...
#ifdef CONFIG_IEEE80211R
  if (!(wpa_s->drv_flags &
        (WPA_DRIVER_FLAGS_SME | WPA_DRIVER_FLAGS_UPDATE_FT_IES)))
    sel &= ~WPA_KEY_MGMT_FT;
#endif /* CONFIG_IEEE80211R */
  wpa_dbg(wpa_s, MSG_DEBUG,
          "WPA: AP key_mgmt 0x%x network profile key_mgmt 0x%x; available "
          "key_mgmt 0x%x",
          ie.key_mgmt, ssid->key_mgmt, sel);
  if (0) {
#ifdef CONFIG_IEEE80211R
    ...
    wpa_dbg(wpa_s, MSG_DEBUG, "WPA: using KEY_MGMT 802.1X with SHA384");
#endif /* CONFIG_SHA384 */
#ifdef CONFIG_FILS
#ifdef CONFIG_IEEE80211R
    ...
    wpa_dbg(wpa_s, MSG_DEBUG, "RSN: using KEY_MGMT FT/SAE (ext key)");
  ...
  if (!(ie.capabilities & WPA_CAPABILITY_MFPC) &&
    ...
    wpa_msg(wpa_s, MSG_INFO,
            "RSN: Management frame protection required but the selected AP "
            "does not enable it");
    return -1;
  }
...
#ifdef CONFIG_SAE
  sae_pwe = wpas_get_ssid_sae_pwe(wpa_s, ssid);
  if ((ssid->sae_password_id || wpa_key_mgmt_sae_ext_key(wpa_s->key_mgmt)) &&
      sae_pwe != SAE_PWE_FORCE_HUNT_AND_PECK)
    sae_pwe = SAE_PWE_HASH_TO_ELEMENT;
  if (bss && is_6ghz_freq(bss->freq) && sae_pwe == SAE_PWE_HUNT_AND_PECK) {
    wpa_dbg(wpa_s, MSG_DEBUG,
  ...
  if (bss && is_6ghz_freq(bss->freq) &&
    ...
    wpa_sm_set_param(wpa_s->wpa, WPA_PARAM_MFP, MGMT_FRAME_PROTECTION_REQUIRED);
  }
#ifdef CONFIG_TESTING_OPTIONS
  ...
  wpa_sm_set_param(wpa_s->wpa, WPA_PARAM_EAPOL_2_KEY_INFO_SET_MASK,
  ...
  if (ssid->mode == WPAS_MODE_INFRA && wpa_s->conf->extended_key_id &&
    ...
    if (bss_rsn && wpa_s->conf->extended_key_id &&
  ...
  wmm =
      wpa_s->connection_set && (wpa_s->connection_ht || wpa_s->connection_vht ||
                                wpa_s->connection_he || wpa_s->connection_eht);
  if (!wmm && bss) wmm = !!wpa_bss_get_vendor_ie(bss, WMM_IE_VENDOR_TYPE);
  ...
  if (ssid->ssid_protection && proto == WPA_PROTO_RSN) {
    ...
    ssid_prot =
        ieee802_11_rsnx_capab(bss_rsnx, WLAN_RSNX_CAPAB_SSID_PROTECTION);
  ...
  if (0) {
    ...
    if (!psk_set && !ssid->pmk_valid) {
      wpa_msg(wpa_s, MSG_INFO, "No PSK/PMK available for association");
  ...
  switch (idx) {
    ...
    case 2: /* Bits 16-23 */
      ...
      *pos |= 0x02; /* Bit 17 - WNM-Sleep Mode */
      if ((wpas_driver_bss_selection(wpa_s) || !wpa_s->disable_mbo_oce) &&
    ...
    case 3: /* Bits 24-31 */
      ...
      *pos |= 0x02; /* Bit 25 - SSID List */
      ...
      if (wpa_s->conf->interworking) *pos |= 0x80; /* Bit 31 - Interworking */
    ...
    case 5: /* Bits 40-47 */
#ifdef CONFIG_HS20
      if (wpa_s->conf->hs20) *pos |= 0x40; /* Bit 46 - WNM-Notification */
#endif                                     /* CONFIG_HS20 */
#ifdef CONFIG_MBO
    ...
    case 6: /* Bits 48-55 */
#ifndef CONFIG_NO_ROBUST_AV
#ifdef CONFIG_TESTING_OPTIONS
      if (wpa_s->disable_scs_support) scs = false;
#endif /* CONFIG_TESTING_OPTIONS */
      ...
      if (scs) *pos |= 0x40; /* Bit 54 - SCS */
#endif                       /* CONFIG_NO_ROBUST_AV */
      break;
    case 7: /* Bits 56-63 */
      break;
    case 8:                                         /* Bits 64-71 */
      if (wpa_s->conf->ftm_responder) *pos |= 0x40; /* Bit 70 - FTM responder */
      if (wpa_s->conf->ftm_initiator) *pos |= 0x80; /* Bit 71 - FTM initiator */
      break;
    case 9: /* Bits 72-79 */
#ifdef CONFIG_FILS
      if (!wpa_s->disable_fils) *pos |= 0x01;
#endif /* CONFIG_FILS */
    ...
    case 10: /* Bits 80-87 */
#ifndef CONFIG_NO_ROBUST_AV
#ifdef CONFIG_TESTING_OPTIONS
      if (wpa_s->disable_mscs_support) mscs = false;
#endif /* CONFIG_TESTING_OPTIONS */
      ...
      if (mscs) *pos |= 0x20; /* Bit 85 - Mirrored SCS */
#endif                        /* CONFIG_NO_ROBUST_AV */
      break;
  }
}
...
int wpas_build_ext_capab(struct wpa_supplicant *wpa_s, u8 *buf, size_t buflen,
  ...
  if (len < wpa_s->extended_capa_len) len = wpa_s->extended_capa_len;
...
int wpas_valid_bss_ssid(struct wpa_supplicant *wpa_s, struct wpa_bss *test_bss,
                        struct wpa_ssid *test_ssid) {
  if (test_bss && !wpas_valid_bss(wpa_s, test_bss)) return 0;

  return test_ssid == NULL || wpas_valid_ssid(wpa_s, test_ssid);
}

void wpas_connect_work_free(struct wpa_connect_work *cwork) {
  if (cwork == NULL) return;
  os_free(cwork);
}
...
int wpas_update_random_addr(struct wpa_supplicant *wpa_s,
  ...
  switch (style) {
    case WPAS_MAC_ADDR_STYLE_RANDOM:
      if (random_mac_addr(addr) < 0) return -1;
      break;
    case WPAS_MAC_ADDR_STYLE_RANDOM_SAME_OUI:
      os_memcpy(addr, wpa_s->perm_addr, ETH_ALEN);
      if (random_mac_addr_keep_oui(addr) < 0) return -1;
      break;
    case WPAS_MAC_ADDR_STYLE_DEDICATED_PER_ESS:
      if (!ssid) {
        wpa_msg(wpa_s, MSG_INFO, "Invalid 'ssid' for address policy 3");
        return -1;
      }
...
void wpa_s_setup_sae_pt(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid,
  ...
  if (!groups || groups[0] <= 0) groups = default_groups;

  password = ssid->sae_password;
  if (!password) password = ssid->passphrase;

  sae_pwe = wpas_get_ssid_sae_pwe(wpa_s, ssid);

  if (!password || !wpa_key_mgmt_sae(ssid->key_mgmt) ||
  ...
  if (ssid->pt) return; /* PT already derived */
  ssid->pt =
      sae_derive_pt(groups, ssid->ssid, ssid->ssid_len, (const u8 *)password,
                    os_strlen(password), ssid->sae_password_id);
#endif /* CONFIG_SAE */
}
...
void wpa_supplicant_associate(struct wpa_supplicant *wpa_s, struct wpa_bss *bss,
  ...
  if (clear_rejected) wpa_s_clear_sae_rejected(wpa_s);
  ...
  if (rand_style > WPAS_MAC_ADDR_STYLE_PERMANENT) {
    ...
    if (wpas_restore_permanent_mac_addr(wpa_s) < 0) return;
  }
  ...
  if (ssid->mode == WPAS_MODE_MESH) {
    ...
    if (bss) ssid->frequency = bss->freq;
...
#ifdef CONFIG_DPP
  if (wpas_dpp_check_connect(wpa_s, ssid, bss) != 0) return;
#endif /* CONFIG_DPP */

#ifdef CONFIG_TDLS
  if (bss) wpa_tdls_ap_ies(wpa_s->wpa, wpa_bss_ie_ptr(bss), bss->ie_len);
#endif /* CONFIG_TDLS */
...
static int drv_supports_vht(struct wpa_supplicant *wpa_s,
  ...
  if (hw_mode == NUM_HOSTAPD_MODES) return 0;
...
static bool ibss_mesh_is_80mhz_avail(int channel,
...
static bool ibss_mesh_can_use_ht(struct wpa_supplicant *wpa_s,
  ...
  if (!ht_supported(mode)) return false;

#ifdef CONFIG_HT_OVERRIDES
  if (ssid->disable_ht) return false;
...
static bool ibss_mesh_can_use_vht(struct wpa_supplicant *wpa_s,
                                  const struct wpa_ssid *ssid,
                                  struct hostapd_hw_modes *mode) {
  if (mode->mode != HOSTAPD_MODE_IEEE80211A) return false;

  if (!drv_supports_vht(wpa_s, ssid)) return false;
  ...
  if (!vht_supported(mode)) return false;

#ifdef CONFIG_VHT_OVERRIDES
  if (ssid->disable_vht) return false;
...
static bool ibss_mesh_can_use_he(struct wpa_supplicant *wpa_s,
  ...
  if (ssid->disable_he) return false;
...
static bool ibss_mesh_can_use_eht(struct wpa_supplicant *wpa_s,
                                  ...
                                  const struct hostapd_hw_modes *mode,
                                  int ieee80211_mode) {
  if (ssid->disable_eht) return false;
...
static void ibss_mesh_select_40mhz(struct wpa_supplicant *wpa_s,
  ...
  int i, res;
  unsigned int j;
  static const int ht40plus[] = {36,  44,  52,  60,  100, 108, 116, 124,
                                 132, 140, 149, 157, 165, 173, 184, 192};
  int ht40 = -1;

  if (!freq->ht_enabled) return;

  for (chan_idx = 0; chan_idx < mode->num_channels; chan_idx++) {
    pri_chan = &mode->channels[chan_idx];
    if (pri_chan->chan == freq->channel) break;
    pri_chan = NULL;
  }
  if (!pri_chan) return;

  /* Check primary channel flags */
  if (pri_chan->flag & (HOSTAPD_CHAN_DISABLED | HOSTAPD_CHAN_NO_IR)) return;

#ifdef CONFIG_HT_OVERRIDES
  if (ssid->disable_ht40) return;
#endif
  ...
  for (i = 0; i < mode->num_channels; i++) {
    ...
    if (sec_chan->chan == freq->channel + ht40 * 4) break;
    sec_chan = NULL;
  }
  if (!sec_chan) return;

  /* Check secondary channel flags */
  if (sec_chan->flag & (HOSTAPD_CHAN_DISABLED | HOSTAPD_CHAN_NO_IR)) return;

  if (ht40 == -1) {
    if (!(pri_chan->flag & HOSTAPD_CHAN_HT40MINUS)) return;
  } else {
    if (!(pri_chan->flag & HOSTAPD_CHAN_HT40PLUS)) return;
  }
...
static bool ibss_mesh_select_80_160mhz(struct wpa_supplicant *wpa_s,
  ...
  static const int bw80[] = {5180, 5260, 5500, 5580, 5660, 5745, 5825,
                             5955, 6035, 6115, 6195, 6275, 6355, 6435,
                             6515, 6595, 6675, 6755, 6835, 6915, 6995};
  static const int bw160[] = {5955, 6115, 6275, 6435, 6595, 6755, 6915};
  ...
  if (!freq->vht_enabled && !freq->he_enabled) return true;
  ...
  if (freq->sec_channel_offset == 0) {
    ...
    if (freq->ht_enabled && !is_6ghz) goto skip_80mhz;
  }
  if (ssid->max_oper_chwidth == CONF_OPER_CHWIDTH_USE_HT) goto skip_80mhz;

  /* setup center_freq1, bandwidth */
  for (j = 0; j < ARRAY_SIZE(bw80); j++) {
    if (freq->freq >= bw80[j] && freq->freq < bw80[j] + 80) break;
  }
  ...
  if (!ibss_mesh_is_80mhz_avail(channel, mode)) goto skip_80mhz;
  ...
  if ((mode->he_capab[ieee80211_mode].phy_cap[HE_PHYCAP_CHANNEL_WIDTH_SET_IDX] &
      ...
      is_6ghz && ibss_mesh_is_80mhz_avail(channel + 16, mode)) {
   ...
   */
  if ((mode->eht_capab[ieee80211_mode]
           .phy_cap[EHT_PHYCAP_320MHZ_IN_6GHZ_SUPPORT_IDX] &
       EHT_PHYCAP_320MHZ_IN_6GHZ_SUPPORT_MASK) &&
      is_6ghz && ibss_mesh_is_80mhz_avail(channel + 16, mode) &&
      ibss_mesh_is_80mhz_avail(channel + 32, mode) &&
      ibss_mesh_is_80mhz_avail(channel + 48, mode)) {
    for (j = 0; j < ARRAY_SIZE(bw320); j += 2) {
      if (freq->freq >= bw320[j] && freq->freq <= bw320[j + 1]) {
  ...
  if (ssid->max_oper_chwidth == CONF_OPER_CHWIDTH_80P80MHZ) {
    ...
    for (k = 0; k < ARRAY_SIZE(bw80); k++) {
      ...
      if (j == k || abs(bw80[j] - bw80[k]) == 80) continue;

      if (ieee80211_freq_to_chan(bw80[k], &channel) == NUM_HOSTAPD_MODES) break;
      ...
      for (i = channel; i < channel + 16; i += 4) {
        ...
        if (!is_6ghz) vht_caps |= VHT_CAP_SUPP_CHAN_WIDTH_160_80PLUS80MHZ;
        seg1 = channel + 6;
      }

      if (chwidth == CONF_OPER_CHWIDTH_80P80MHZ) break;
    }
...
skip_80mhz:
  if (hostapd_set_freq_params(
          &vht_freq, mode->mode, freq->freq, freq->channel, ssid->enable_edmg,
          ssid->edmg_channel, freq->ht_enabled, freq->vht_enabled,
          freq->he_enabled, freq->eht_enabled, freq->sec_channel_offset,
          chwidth, seg0, seg1, vht_caps, &mode->he_capab[ieee80211_mode],
...
void ibss_mesh_setup_freq(struct wpa_supplicant *wpa_s,
  ...
  is_24ghz =
      hw_mode == HOSTAPD_MODE_IEEE80211G || hw_mode == HOSTAPD_MODE_IEEE80211B;
  ...
  if (!is_6ghz) freq->ht_enabled = ibss_mesh_can_use_ht(wpa_s, ssid, mode);
  if (freq->ht_enabled)
    freq->vht_enabled = ibss_mesh_can_use_vht(wpa_s, ssid, mode);
  if (freq->vht_enabled || (freq->ht_enabled && is_24ghz) || is_6ghz)
    freq->he_enabled = ibss_mesh_can_use_he(wpa_s, ssid, mode, ieee80211_mode);
  freq->channel = channel;
  /* Setup higher BW only for 5 GHz */
  if (mode->mode == HOSTAPD_MODE_IEEE80211A) {
    ibss_mesh_select_40mhz(wpa_s, ssid, mode, freq, obss_scan);
    if (!ibss_mesh_select_80_160mhz(wpa_s, ssid, mode, freq, ieee80211_mode,
                                    is_6ghz))
  ...
  if (freq->he_enabled)
    freq->eht_enabled =
        ibss_mesh_can_use_eht(wpa_s, ssid, mode, ieee80211_mode);
}
...
static size_t wpas_add_fils_hlp_req(struct wpa_supplicant *wpa_s, u8 *ie_buf,
  ...
  dl_list_for_each(req, &wpa_s->fils_hlp_req, struct fils_hlp_req, list) {
    ...
    if (rem_len < 2 + hdr_len + hlp_len) {
      wpa_printf(MSG_ERROR, "FILS: Cannot fit HLP - rem_len=%lu to_fill=%lu",
...
bool wpa_is_non_eht_scs_traffic_desc_supported(struct wpa_bss *bss) {
  ...
  if (!wfa_capa)
    wfa_capa = wpa_bss_get_vendor_ie_beacon(bss, WFA_CAPA_IE_VENDOR_TYPE);
...
int wpas_populate_wfa_capa(struct wpa_supplicant *wpa_s, struct wpa_bss *bss,
  ...
  if (attr) wpabuf_put_buf(wfa_ie, attr);
  wpabuf_free(attr);
...
static u8 *wpas_populate_assoc_ies(
    struct wpa_supplicant *wpa_s, struct wpa_bss *bss, struct wpa_ssid *ssid,
  ...
  dl_list_for_each(req, &wpa_s->fils_hlp_req, struct fils_hlp_req, list) {
    max_wpa_ie_len += 3 + 2 * ETH_ALEN + 6 + wpabuf_len(req->pkt) + 2 +
                      2 * wpabuf_len(req->pkt) / 255;
  }
#endif /* CONFIG_FILS */

  wpa_ie = os_malloc(max_wpa_ie_len);
  if (!wpa_ie) {
    wpa_printf(MSG_ERROR, "Failed to allocate connect IE buffer for %lu bytes",
  ...
  if (bss &&
    ...
    if ((wpa_s->drv_flags & WPA_DRIVER_FLAGS_SME) && wpa_s->valid_links)
...
#endif /* CONFIG_FILS */
    if (pmksa_cache_set_current(wpa_s->wpa, NULL, addr, ssid, try_opportunistic,
                                cache_id, 0, false) == 0) {
  ...
  if ((wpa_s->drv_flags & WPA_DRIVER_FLAGS_FILS_SK_OFFLOAD) && ssid->eap.erp &&
      wpa_key_mgmt_fils(wpa_s->key_mgmt) &&
      eapol_sm_get_erp_info(wpa_s->eapol, &ssid->eap, &username, &username_len,
                            &realm, &realm_len, &next_seq_num, &rrk,
                            &rrk_len) == 0 &&
    ...
    if (mask) *mask |= WPA_DRV_UPDATE_FILS_ERP_INFO;
...
#endif /* IEEE8021X_EAPOL */
#ifdef CONFIG_SAE
  if (wpa_key_mgmt_sae(wpa_s->key_mgmt)) algs = WPA_AUTH_ALG_SAE;
#endif /* CONFIG_SAE */
...
#ifndef CONFIG_NO_RRM
  if (bss) {
    wpa_ie_len += wpas_supp_op_class_ie(wpa_s, ssid, bss, wpa_ie + wpa_ie_len,
  ...
  if (!bss || wpa_bss_get_ie(bss, WLAN_EID_EXT_CAPAB)) {
    ...
    int ext_capab_len;
    ext_capab_len =
        wpas_build_ext_capab(wpa_s, ext_capab, sizeof(ext_capab), bss);
...
#ifdef CONFIG_DPP2
  if (DPP_VERSION > 1 && wpa_sm_get_key_mgmt(wpa_s->wpa) == WPA_KEY_MGMT_DPP &&
      ssid->dpp_netaccesskey && ssid->dpp_pfs != 2 && !ssid->dpp_pfs_fallback) {
    struct rsn_pmksa_cache_entry *pmksa;

    pmksa = pmksa_cache_get_current(wpa_s->wpa);
    if (!pmksa || !pmksa->dpp_pfs) goto pfs_fail;

    dpp_pfs_free(wpa_s->dpp_pfs);
    wpa_s->dpp_pfs =
        dpp_pfs_init(ssid->dpp_netaccesskey, ssid->dpp_netaccesskey_len);
    if (!wpa_s->dpp_pfs) {
    ...
    if (wpabuf_len(wpa_s->dpp_pfs->ie) <= max_wpa_ie_len - wpa_ie_len) {
      os_memcpy(wpa_ie + wpa_ie_len, wpabuf_head(wpa_s->dpp_pfs->ie),
...
pfs_fail:
  ...
  if (bss && wpa_key_mgmt_ft(wpa_sm_get_key_mgmt(wpa_s->wpa))) {
    ...
    if (mdie && mdie[1] >= MOBILITY_DOMAIN_ID_LEN) {
      ...
      if (os_memcmp(md, wpa_md, MOBILITY_DOMAIN_ID_LEN) == 0) {
        ...
        len = wpa_ft_add_mdie(wpa_s->wpa, wpa_ie + wpa_ie_len,
                              max_wpa_ie_len - wpa_ie_len, mdie);
        wpa_ie_len += len;
      }
#ifdef CONFIG_SME
      if (len > 0 && wpa_s->sme.ft_used && wpa_sm_has_ft_keys(wpa_s->wpa, md)) {
  ...
  if (wpa_s->rsnxe_override_assoc &&
      wpabuf_len(wpa_s->rsnxe_override_assoc) <= max_wpa_ie_len - wpa_ie_len) {
...
#ifndef CONFIG_NO_ROBUST_AV
#ifdef CONFIG_TESTING_OPTIONS
  if (wpa_s->disable_mscs_support) goto mscs_end;
#endif /* CONFIG_TESTING_OPTIONS */
...
mscs_end:
  ...
  wpa_ie_len =
      wpas_populate_wfa_capa(wpa_s, bss, wpa_ie, wpa_ie_len, max_wpa_ie_len);
  ...
  if (ssid->multi_ap_backhaul_sta) {
    ...
    multi_ap_ie_len = add_multi_ap_ie(wpa_ie + wpa_ie_len,
                                      max_wpa_ie_len - wpa_ie_len, &multi_ap);
  ...
  wpa_sm_set_param(wpa_s->wpa, WPA_PARAM_RSN_OVERRIDE, RSN_OVERRIDE_NOT_USED);
  if (wpas_rsn_overriding(wpa_s, ssid) &&
    ...
    wpa_sm_set_param(wpa_s->wpa, WPA_PARAM_RSN_OVERRIDE, RSN_OVERRIDE_RSNE);
  ...
  if (mask) *mask |= WPA_DRV_UPDATE_ASSOC_IES | WPA_DRV_UPDATE_AUTH_TYPE;
...
static void wpas_update_fils_connect_params(struct wpa_supplicant *wpa_s) {
  ...
  if (wpa_s->auth_alg != WPA_AUTH_ALG_OPEN) return; /* nothing to do */
...
static u8 wpa_ie_get_edmg_oper_chans(const u8 *edmg_ie) {
  if (!edmg_ie || edmg_ie[1] < 6) return 0;
 ...
 */
static struct ieee80211_edmg_config get_edmg_intersection(
    struct ieee80211_edmg_config a, struct ieee80211_edmg_config b,
    u8 primary_channel) {
  ...
  if (!(result.channels & BIT(primary_channel - 1))) {
    wpa_printf(MSG_DEBUG,
               "EDMG not possible: the primary channel %d is not one of the "
               "intersected channels 0x%x",
  ...
  if ((max_contiguous >= 2 && result.bw_config < EDMG_BW_CONFIG_5) ||
    ...
    wpa_printf(MSG_DEBUG,
               "EDMG not possible: not enough contiguous channels %d for "
               "supporting CB1 or CB2",
               max_contiguous);
    goto fail;
  }
...
static struct ieee80211_edmg_config get_supported_edmg(
    struct wpa_supplicant *wpa_s, struct hostapd_freq_params *freq,
    struct ieee80211_edmg_config request_edmg) {
  enum hostapd_hw_mode hw_mode;
  struct hostapd_hw_modes *mode = NULL;
  u8 primary_channel;

  if (!wpa_s->hw.modes) goto fail;

  hw_mode = ieee80211_freq_to_chan(freq->freq, &primary_channel);
  if (hw_mode == NUM_HOSTAPD_MODES) goto fail;
...
static void wpas_start_assoc_cb(struct wpa_radio_work *work, int deinit) {
   ...
   * will be zero.
   */
  os_memcpy(prev_bssid, wpa_s->valid_links ? wpa_s->ap_mld_addr : wpa_s->bssid,
            ETH_ALEN);
  os_memset(&params, 0, sizeof(params));
  wpa_s->reassociate = 0;
  ...
  if (bss && (!wpas_driver_bss_selection(wpa_s) || wpas_wps_searching(wpa_s))) {
    ...
    if (ie && ie[1] >= MOBILITY_DOMAIN_ID_LEN) md = ie + 2;
  ...
  if (!wpa_s->pno) wpa_supplicant_cancel_sched_scan(wpa_s);
  ...
  if (wpa_s->key_mgmt == WPA_KEY_MGMT_NONE ||
      wpa_s->key_mgmt == WPA_KEY_MGMT_IEEE8021X_NO_WPA) {
    if (wpa_s->key_mgmt == WPA_KEY_MGMT_NONE) use_crypt = 0;
#ifdef CONFIG_WEP
  ...
  if (bss) {
    ...
    if (ssid->bssid_hint_set) params.bssid_hint = ssid->bssid_hint;
  ...
  if (edmg_ie_oper) {
    ...
    wpa_printf(MSG_DEBUG, "AP supports EDMG channels 0x%x, bw_config %d",
               params.freq.edmg.channels, params.freq.edmg.bw_config);
    ...
    if (params.freq.edmg.channels) {
      wpa_printf(MSG_DEBUG, "EDMG before: channels 0x%x, bw_config %d",
                 params.freq.edmg.channels, params.freq.edmg.bw_config);
      params.freq.edmg =
          get_supported_edmg(wpa_s, &params.freq, params.freq.edmg);
      wpa_printf(MSG_DEBUG, "EDMG after: channels 0x%x, bw_config %d",
                 params.freq.edmg.channels, params.freq.edmg.bw_config);
    }
  }
...
#ifdef CONFIG_WEP
  {
    ...
    for (i = 0; i < NUM_WEP_KEYS; i++) {
      if (ssid->wep_key_len[i]) params.wep_key[i] = ssid->wep_key[i];
...
#elif (defined(CONFIG_DRIVER_NL80211_BRCM) &&       \
       defined(WIFI_BRCM_OPEN_SOURCE_MULTI_AKM)) || \
    defined(CONFIG_DRIVER_NL80211_SYNA)
      (params.key_mgmt_suite == WPA_KEY_MGMT_PSK ||
       params.key_mgmt_suite == WPA_KEY_MGMT_FT_PSK ||
  ...
  if (wpa_s->reassoc_same_ess && !is_zero_ether_addr(prev_bssid) && old_ssid)
...
#ifndef CONFIG_NO_WPA
  if (bss) wpa_sm_set_ssid(wpa_s->wpa, bss->ssid, bss->ssid_len);
#endif /* CONFIG_NO_WPA */
  wpa_supplicant_initiate_eapol(wpa_s);
  if (old_ssid != wpa_s->current_ssid) wpas_notify_network_changed(wpa_s);
  if (!(wpa_s->drv_flags & WPA_DRIVER_FLAGS_SME))
    wpas_notify_auth_changed(wpa_s);
}
...
void wpa_supplicant_deauthenticate(struct wpa_supplicant *wpa_s,
  ...
  if (wpa_s->ifmsh) {
    ...
    wpas_notify_mesh_group_removed(wpa_s, mconf->meshid, mconf->meshid_len,
                                   reason_code);
...
static void wpa_supplicant_enable_one_network(struct wpa_supplicant *wpa_s,
                                              struct wpa_ssid *ssid) {
  if (!ssid || !ssid->disabled || ssid->disabled == 2) return;
   ...
   */
  if (!wpa_s->current_ssid && !wpa_s->disconnected) wpa_s->reassociate = 1;
}
...
int wpa_supplicant_remove_network(struct wpa_supplicant *wpa_s, int id) {
  ...
  if (wpa_config_remove_network(wpa_s->conf, id) < 0) return -2;
...
int wpa_supplicant_remove_all_networks(struct wpa_supplicant *wpa_s) {
  ...
  if (wpa_s->drv_flags2 &
      (WPA_DRIVER_FLAGS2_SAE_OFFLOAD_STA | WPA_DRIVER_FLAGS2_OWE_OFFLOAD_STA))
    wpa_drv_flush_pmkid(wpa_s);

  if (wpa_s->sched_scanning) wpa_supplicant_cancel_sched_scan(wpa_s);
...
static bool ssid_in_last_scan(struct wpa_supplicant *wpa_s,
  ...
  if (wpa_s->last_scan_num_ssids <= 1 || !ssid->ssid || ssid->ssid_len == 0)
    return false;

  /* Iterate through the previous scan SSIDs */
  for (i = 0; i < wpa_s->last_scan_num_ssids; i++) {
    if (os_memcmp(wpa_s->last_scan_ssids[i].ssid, ssid->ssid, ssid->ssid_len) ==
        0)
      return true;
  }
...
static bool ssid_in_last_scan_res(struct wpa_supplicant *wpa_s,
  ...
  if (!wpa_s->last_scan_res || !ssid->ssid || ssid->ssid_len == 0) return false;

  for (i = 0; i < wpa_s->last_scan_res_used; i++) {
    if (os_memcmp(wpa_s->last_scan_res[i]->ssid, ssid->ssid, ssid->ssid_len) ==
        0)
      return true;
  }
...
void wpa_supplicant_select_network(struct wpa_supplicant *wpa_s,
  ...
  if (ssid) wpas_clear_temp_disabled(wpa_s, ssid, 1);
  ...
  if (ssid) {
    ...
    wpa_s->connect_without_scan =
        (ssid->mode == WPAS_MODE_MESH || ssid->mode == WPAS_MODE_AP) ? ssid
                                                                     : NULL;

    if (ssid->scan_ssid) {
      if (ssid_in_last_scan(wpa_s, ssid)) {
        wpa_printf(MSG_DEBUG, "Hidden network was scanned for in last scan");
      } else if (ssid_in_last_scan_res(wpa_s, ssid)) {
        wpa_printf(MSG_DEBUG, "Hidden network was found in last scan results");
      } else {
    ...
    if (!request_new_scan && (ssid->key_mgmt & WPA_KEY_MGMT_OWE) &&
        !ssid->owe_only) {
      wpa_printf(MSG_DEBUG, "Request a new scan for OWE transition SSID");
      request_new_scan = true;
    }
  ...
  if (ssid) wpas_notify_network_selected(wpa_s, ssid);
}
...
int wpas_remove_all_creds(struct wpa_supplicant *wpa_s) {
  ...
  while (cred) {
    ...
    if (res < 0) {
      wpa_printf(MSG_DEBUG,
                 "Removal of all credentials failed - failed to remove "
                 "credential id=%d",
                 prev->id);
      ret = -1;
    }
...
int wpas_set_pkcs11_engine_and_module_path(struct wpa_supplicant *wpa_s,
  ...
  if (pkcs11_engine_path != NULL) {
    ...
    if (pkcs11_engine_path_copy == NULL) return -1;
  }
...
int wpa_supplicant_set_ap_scan(struct wpa_supplicant *wpa_s, int ap_scan) {
  ...
  if (ap_scan < 0 || ap_scan > 2) return -1;
...
int wpa_supplicant_set_debug_params(struct wpa_global *global, int debug_level,
  ...
  if (wpa_debug_level != old_level) wpas_notify_debug_level_changed(global);
...
static int owe_trans_ssid_match(struct wpa_supplicant *wpa_s, const u8 *bssid,
  ...
  if (wpas_get_owe_trans_network(owe, &owe_bssid, &owe_ssid, &owe_ssid_len))
    return 0;
  ...
  while (entry) {
    if (!wpas_network_disabled(wpa_s, entry) &&
        ((ssid_len == entry->ssid_len &&
          (!entry->ssid || os_memcmp(ssid, entry->ssid, ssid_len) == 0)) ||
         wired) &&
        (!entry->bssid_set || ether_addr_equal(bssid, entry->bssid)))
      return entry;
#ifdef CONFIG_WPS
    if (!wpas_network_disabled(wpa_s, entry) &&
    ...
    if (!wpas_network_disabled(wpa_s, entry) &&
        (entry->ssid &&
         owe_trans_ssid_match(wpa_s, bssid, entry->ssid, entry->ssid_len)) &&
        (!entry->bssid_set || ether_addr_equal(bssid, entry->bssid)))
      return entry;
#endif /* CONFIG_OWE */

    if (!wpas_network_disabled(wpa_s, entry) && entry->bssid_set &&
        entry->ssid_len == 0 && ether_addr_equal(bssid, entry->bssid))
      return entry;
...
static int wpa_supplicant_set_driver(struct wpa_supplicant *wpa_s,
  ...
  if (name == NULL) {
    ...
    for (i = 0; wpa_drivers[i]; i++) {
      if (select_driver(wpa_s, i) == 0) return 0;
    }
...
void wpa_supplicant_rx_eapol(void *ctx, const u8 *src_addr, const u8 *buf,
...
#ifdef CONFIG_TESTING_OPTIONS
  wpa_msg_ctrl(wpa_s, MSG_INFO, "EAPOL-RX " MACSTR " %zu", MAC2STR(src_addr),
               len);
  ...
  wpa_s->last_eapol_matches_bssid = ether_addr_equal(src_addr, connected_addr);

#ifdef CONFIG_AP
  if (wpa_s->ap_iface) {
    wpa_supplicant_ap_rx_eapol(wpa_s, src_addr, buf, len, encrypted);
    return;
  }
#endif /* CONFIG_AP */
  ...
  if (wpa_s->current_ssid && wpa_s->current_ssid->mode == WPAS_MODE_IBSS) {
    ibss_rsn_rx_eapol(wpa_s->ibss_rsn, src_addr, buf, len, encrypted);
    return;
  }
#endif /* CONFIG_IBSS_RSN */
  ...
  if (!wpa_key_mgmt_wpa_psk(wpa_s->key_mgmt) &&
      wpa_s->key_mgmt != WPA_KEY_MGMT_OWE &&
      wpa_s->key_mgmt != WPA_KEY_MGMT_DPP &&
      eapol_sm_rx_eapol(wpa_s->eapol, src_addr, buf, len, encrypted) > 0)
    return;
  wpa_drv_poll(wpa_s);
  if (!(wpa_s->drv_flags & WPA_DRIVER_FLAGS_4WAY_HANDSHAKE_PSK)) {
    if (wpa_sm_rx_eapol(wpa_s->wpa, src_addr, buf, len, encrypted) == -2 &&
#ifdef CONFIG_AP
        !wpa_s->ap_iface &&
#endif /* CONFIG_AP */
        wpa_s->last_eapol_matches_bssid) {
      ...
      wpa_dbg(wpa_s, MSG_DEBUG,
              "Delay processing of received EAPOL frame for reassociation to "
              "the current connected AP (state=%s connected_addr=" MACSTR ")",
              wpa_supplicant_state_txt(wpa_s->wpa_state),
              MAC2STR(connected_addr));
...
int wpa_supplicant_update_mac_addr(struct wpa_supplicant *wpa_s) {
  ...
  if ((!wpa_s->p2p_mgmt ||
    ...
    l2_packet_deinit(wpa_s->l2);
    wpa_s->l2 = l2_packet_init(
        wpa_s->ifname, wpa_drv_get_mac_addr(wpa_s), ETH_P_EAPOL,
        wpas_eapol_needs_l2_packet(wpa_s) ? wpa_supplicant_rx_eapol_cb : NULL,
        wpa_s, 0);
    if (wpa_s->l2 == NULL) return -1;
    ...
    if (addr) os_memcpy(wpa_s->own_addr, addr, ETH_ALEN);
  }
...
#ifdef CONFIG_FST
  if (wpa_s->fst) fst_update_mac_addr(wpa_s->fst, wpa_s->own_addr);
#endif /* CONFIG_FST */
...
int wpa_supplicant_update_bridge_ifname(struct wpa_supplicant *wpa_s,
  ...
  if (wpa_s->wpa_state > WPA_SCANNING) return -EBUSY;

  if (bridge_ifname && os_strlen(bridge_ifname) >= sizeof(wpa_s->bridge_ifname))
    return -EINVAL;

  if (!bridge_ifname) bridge_ifname = "";

  if (os_strcmp(wpa_s->bridge_ifname, bridge_ifname) == 0) return 0;
  ...
  if (wpa_s->bridge_ifname[0]) {
    wpa_dbg(wpa_s, MSG_DEBUG, "Receiving packets from bridge interface '%s'",
            wpa_s->bridge_ifname);
...
#ifdef CONFIG_TDLS
  if (!wpa_s->p2p_mgmt && wpa_tdls_init(wpa_s->wpa)) goto fail;
#endif /* CONFIG_TDLS */
...
static struct wpa_supplicant *wpa_supplicant_alloc(
    struct wpa_supplicant *parent) {
  struct wpa_supplicant *wpa_s;

  wpa_s = os_zalloc(sizeof(*wpa_s));
  if (wpa_s == NULL) return NULL;
...
#ifdef CONFIG_P2P
  if (parent) wpa_s->p2p_mode = parent->p2p_mode;
#endif /* CONFIG_P2P */
...
static int wpa_set_disable_ht40(struct wpa_supplicant *wpa_s,
                                ...
                                struct ieee80211_ht_capabilities *htcaps_mask,
                                int disabled) {
  if (disabled) wpa_msg(wpa_s, MSG_DEBUG, "set_disable_ht40: %d", disabled);
...
static int wpa_set_disable_sgi(struct wpa_supplicant *wpa_s,
  ...
  if (disabled) wpa_msg(wpa_s, MSG_DEBUG, "set_disable_sgi: %d", disabled);
...
static int wpa_set_disable_ldpc(struct wpa_supplicant *wpa_s,
  ...
  if (disabled) wpa_msg(wpa_s, MSG_DEBUG, "set_disable_ldpc: %d", disabled);
...
static int wpa_set_tx_stbc(struct wpa_supplicant *wpa_s,
  ...
  if (tx_stbc == -1) return 0;

  wpa_msg(wpa_s, MSG_DEBUG, "set_tx_stbc: %d", tx_stbc);

  if (tx_stbc < 0 || tx_stbc > 1) {
    wpa_msg(wpa_s, MSG_ERROR, "tx_stbc: %d out of range. Must be 0-1 or -1",
            tx_stbc);
    return -EINVAL;
  }
...
static int wpa_set_rx_stbc(struct wpa_supplicant *wpa_s,
  ...
  if (rx_stbc == -1) return 0;

  wpa_msg(wpa_s, MSG_DEBUG, "set_rx_stbc: %d", rx_stbc);

  if (rx_stbc < 0 || rx_stbc > 3) {
    wpa_msg(wpa_s, MSG_ERROR, "rx_stbc: %d out of range. Must be 0-3 or -1",
            rx_stbc);
    return -EINVAL;
  }
...
void wpa_supplicant_apply_ht_overrides(
  ...
  if (!params->htcaps || !params->htcaps_mask) return;
...
void wpa_supplicant_apply_vht_overrides(
  ...
  if (!vhtcaps || !vhtcaps_mask) return;
...
static int pcsc_reader_init(struct wpa_supplicant *wpa_s) {
  ...
  if (!wpa_s->conf->pcsc_reader) return 0;

  wpa_s->scard = scard_init(wpa_s->conf->pcsc_reader);
  if (!wpa_s->scard) return 1;
...
int wpas_init_ext_pw(struct wpa_supplicant *wpa_s) {
  ...
  if (!wpa_s->conf->ext_password_backend) return 0;
...
static int wpas_fst_send_action_cb(void *ctx, const u8 *da,
  ...
  if (!ether_addr_equal(wpa_s->bssid, da)) {
    wpa_printf(MSG_INFO, "FST:%s:bssid=" MACSTR " != da=" MACSTR, __func__,
               MAC2STR(wpa_s->bssid), MAC2STR(da));
    return -1;
  }
...
static int wpas_set_wowlan_triggers(struct wpa_supplicant *wpa_s,
  ...
  if (!wpa_s->conf->wowlan_triggers) return 0;
...
enum wpa_radio_work_band wpas_freq_to_band(int freq) {
  if (freq < 3000) return BAND_2_4_GHZ;
  if (freq > 50000) return BAND_60_GHZ;
  return BAND_5_GHZ;
}
...
unsigned int wpas_get_bands(struct wpa_supplicant *wpa_s, const int *freqs) {
  ...
  if (freqs) {
    ...
    for (i = 0; freqs[i]; i++) band |= wpas_freq_to_band(freqs[i]);
  } else {
    /*
...
static struct wpa_radio *radio_add_interface(struct wpa_supplicant *wpa_s,
  ...
  if (rn) os_strlcpy(radio->name, rn, sizeof(radio->name));
...
static struct wpa_radio_work *radio_work_get_next_work(
    struct wpa_radio *radio) {
  ...
  dl_list_for_each(tmp, &radio->work, struct wpa_radio_work, list) {
    if (tmp->started) continue;

    /*
     * If connect or sme-connect are enqueued, parallelize only
     * those operations ahead of them in the queue.
     */
    if (radio_work_is_connect(tmp)) break;
    ...
    if (active_work->wpa_s == tmp->wpa_s && radio_work_is_scan(active_work) &&
        radio_work_is_scan(tmp)) {
      wpa_dbg(
          active_work->wpa_s, MSG_DEBUG,
...
static void radio_start_next_work(void *eloop_ctx, void *timeout_ctx) {
  ...
  if (!(wpa_s && wpa_s->drv_flags & WPA_DRIVER_FLAGS_OFFCHANNEL_SIMULTANEOUS)) {
    if (work->started) return; /* already started and still in progress */
...
void radio_remove_works(struct wpa_supplicant *wpa_s, const char *type,
  ...
  dl_list_for_each_safe(work, tmp, &radio->work, struct wpa_radio_work, list) {
    if (type && os_strcmp(type, work->type) != 0) continue;

    /* skip other ifaces' works */
    if (!remove_all && work->wpa_s != wpa_s) continue;
...
void radio_remove_pending_work(struct wpa_supplicant *wpa_s, void *ctx) {
  ...
  dl_list_for_each(work, &radio->work, struct wpa_radio_work, list) {
    if (work->ctx != ctx) continue;
    wpa_dbg(wpa_s, MSG_DEBUG, "Free pending radio work '%s'@%p%s", work->type,
            work, work->started ? " (started)" : "");
    radio_work_free(work);
    break;
  }
}
...
void radio_work_check_next(struct wpa_supplicant *wpa_s) {
  ...
  if (dl_list_empty(&radio->work)) return;
...
void radio_work_done(struct wpa_radio_work *work) {
  ...
  radio_work_free(work);
  if (started) radio_work_check_next(wpa_s);
}
...
struct wpa_radio_work *radio_work_pending(struct wpa_supplicant *wpa_s,
  ...
  dl_list_for_each(work, &radio->work, struct wpa_radio_work, list) {
    if (work->wpa_s == wpa_s && os_strcmp(work->type, type) == 0) return work;
  }
...
next_driver:
...
#ifdef CONFIG_MATCH_IFACE
    if (wpa_s->matched == WPA_IFACE_MATCHED_NULL) level = MSG_DEBUG;
#endif /* CONFIG_MATCH_IFACE */
  ...
  if (rn && rn[0] == '\0') rn = NULL;

  wpa_s->radio = radio_add_interface(wpa_s, rn);
  if (wpa_s->radio == NULL) return -1;
...
static void wpas_gas_server_tx_status(
  ...
  wpa_printf(
      MSG_DEBUG, "GAS: TX status: freq=%u dst=" MACSTR " result=%s", freq,
      MAC2STR(dst),
...
static void wpas_gas_server_tx(void *ctx, int freq, const u8 *da,
  ...
  offchannel_send_action(wpa_s, freq, da, wpa_s->own_addr, broadcast,
                         wpabuf_head(buf), wpabuf_len(buf), wait_time,
...
static int wpa_supplicant_init_iface(struct wpa_supplicant *wpa_s,
  ...
  if (iface->confname) {
    ...
    if (wpa_s->confanother &&
        !wpa_config_read(wpa_s->confanother, wpa_s->conf, true,
                         wpa_s->global->params.show_details)) {
      wpa_printf(MSG_ERROR, "Failed to read or parse configuration '%s'.",
                 wpa_s->confanother);
      return -1;
    }
    ...
    if (iface->ctrl_interface) {
      ...
      if (!wpa_s->conf->ctrl_interface) {
        wpa_printf(MSG_ERROR, "Failed to duplicate control interface '%s'.",
    ...
    if (iface->driver_param) {
      ...
      if (!wpa_s->conf->driver_param) {
        wpa_printf(MSG_ERROR, "Failed to duplicate driver param '%s'.",
                   iface->driver_param);
        return -1;
      }
    }
   ...
   * call. */
  if (wpas_init_driver(wpa_s, iface) < 0) return -1;
  ...
  wpa_s->hw.modes = wpa_drv_get_hw_feature_data(wpa_s, &wpa_s->hw.num_modes,
                                                &wpa_s->hw.flags, &dfs_domain);
  if (wpa_s->hw.modes) {
    u16 i;

    for (i = 0; i < wpa_s->hw.num_modes; i++) {
      if (wpa_s->hw.modes[i].eht_capab[IEEE80211_MODE_INFRA].eht_supported)
        wpa_s->hw_capab |= BIT(CAPAB_EHT);
      if (wpa_s->hw.modes[i].he_capab[IEEE80211_MODE_INFRA].he_supported)
        wpa_s->hw_capab |= BIT(CAPAB_HE);
      if (wpa_s->hw.modes[i].vht_capab) wpa_s->hw_capab |= BIT(CAPAB_VHT);
      if (wpa_s->hw.modes[i].ht_capab) wpa_s->hw_capab |= BIT(CAPAB_HT);
    }
  ...
  if (capa_res == 0) {
    ...
    wpa_drv_get_ext_capa(wpa_s, WPA_IF_STATION);
    if (wpa_s->extended_capa && wpa_s->extended_capa_len >= 3 &&
...
#endif /* CONFIG_PASN */
#ifndef CONFIG_NO_WPA
  wpa_sm_set_driver_bss_selection(
      wpa_s->wpa, !!(wpa_s->drv_flags & WPA_DRIVER_FLAGS_BSS_SELECTION));
#endif /* CONFIG_NO_WPA */
...
#ifdef CONFIG_EAP_PROXY
  {
    size_t len;
    wpa_s->mnc_len =
        eapol_sm_get_eap_proxy_imsi(wpa_s->eapol, -1, wpa_s->imsi, &len);
    if (wpa_s->mnc_len > 0) {
      wpa_s->imsi[len] = '\0';
  ...
  if (pcsc_reader_init(wpa_s) < 0) return -1;

  if (wpas_init_ext_pw(wpa_s) < 0) return -1;
...
static void wpa_supplicant_deinit_iface(struct wpa_supplicant *wpa_s,
  ...
  iface = global->ifaces;
  while (iface) {
    if (iface->p2pdev == wpa_s) iface->p2pdev = iface->parent;
  ...
  if (wpa_s->drv_priv) {
    ...
    if (!wpa_drv_get_wowlan(wpa_s) ||
      ...
      wpa_msg(wpa_s, MSG_INFO,
              "Do not deauthenticate as part of interface deinit since WoWLAN "
              "is enabled");
    }
  }
  ...
  if (wpa_s->drv_priv) wpa_drv_deinit(wpa_s);

  if (notify) wpas_notify_iface_removed(wpa_s);

  if (terminate) wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_TERMINATING);
...
struct wpa_supplicant *wpa_supplicant_add_iface(struct wpa_global *global,
  ...
  if (global == NULL || iface == NULL) return NULL;
...
int wpa_supplicant_remove_iface(struct wpa_global *global,
  ...
  if (prev == wpa_s) {
    ...
    while (prev && prev->next != wpa_s) prev = prev->next;
  ...
  if (global->p2p_group_formation == wpa_s) global->p2p_group_formation = NULL;
  if (global->p2p_invite_group == wpa_s) global->p2p_invite_group = NULL;
...
const char *wpa_supplicant_get_eap_mode(struct wpa_supplicant *wpa_s) {
  ...
  if (eapol_method == NULL) return "UNKNOWN-EAP";
...
struct wpa_supplicant *wpa_supplicant_get_iface(struct wpa_global *global,
  ...
  for (wpa_s = global->ifaces; wpa_s; wpa_s = wpa_s->next) {
    if (os_strcmp(wpa_s->ifname, ifname) == 0) return wpa_s;
  }
  return NULL;
}
...
#ifdef CONFIG_P2P
  if (global->p2p) p2p_expire_peers(global->p2p);
#endif /* CONFIG_P2P */
...
struct wpa_global *wpa_supplicant_init(struct wpa_params *params) {
  ...
  if (params->wpa_debug_syslog) wpa_debug_open_syslog();
  ...
  if (params->match_iface_count) {
    ...
    if (!global->params.match_ifaces) {
      wpa_printf(MSG_ERROR, "Failed to allocate match interfaces");
      wpa_supplicant_deinit(global);
      return NULL;
    }
  ...
  if (params->aidl_service_name)
    global->params.aidl_service_name = os_strdup(params->aidl_service_name);
#endif /* CONFIG_AIDL */
  ...
  for (i = 0; wpa_drivers[i]; i++) global->drv_count++;
...
#ifdef CONFIG_MATCH_IFACE
  if (wpa_supplicant_match_existing(global)) return -1;
#endif
...
void wpa_supplicant_deinit(struct wpa_global *global) {
  ...
  while (global->ifaces) wpa_supplicant_remove_iface(global, global->ifaces, 1);
...
void wpa_supplicant_update_config(struct wpa_supplicant *wpa_s) {
  ...
  if (wpa_s->conf->changed_parameters & CFG_CHANGED_WOWLAN_TRIGGERS) {
    ...
    if (res == 0 && wpas_set_wowlan_triggers(wpa_s, &capa) < 0)
      wpa_printf(MSG_ERROR, "Failed to update wowlan_triggers to '%s'",
...
void fils_pmksa_cache_flush(struct wpa_supplicant *wpa_s) {
  ...
  if (!ssid || !ssid->eap.erp || !wpa_key_mgmt_fils(ssid->key_mgmt)) return;

  if (eapol_sm_get_erp_info(wpa_s->eapol, &ssid->eap, &username, &username_len,
                            &realm, &realm_len, &next_seq_num, &rrk,
                            &rrk_len) != 0 ||
      !realm) {
...
void fils_connection_failure(struct wpa_supplicant *wpa_s) {
  ...
  wpa_hexdump_ascii(MSG_DEBUG, "FILS: Store last connection failure realm",
                    realm, realm_len);
  os_free(wpa_s->last_con_fail_realm);
...
bool wpas_rsn_overriding(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid) {
  ...
  if (rsno == RSN_OVERRIDING_DISABLED) return false;

  if (rsno == RSN_OVERRIDING_ENABLED) return true;
  ...
  return true;
    ...
    defined(CONFIG_CTRL_IFACE_AIDL)
...
int wpa_supplicant_ctrl_rsp_handle(struct wpa_supplicant *wpa_s,
  ...
  switch (rtype) {
    case WPA_CTRL_REQ_EAP_IDENTITY:
      ...
      if (identity == NULL) {
        ...
        *identity++ = '\0';
        /* plain identity */
        eap->imsi_identity =
            (u8 *)dup_binstr(imsi_identity, strlen(imsi_identity));
        eap->imsi_identity_len = strlen(imsi_identity);
        /* encrypted identity */
        eap->identity =
            (u8 *)dup_binstr(identity, value_len - strlen(imsi_identity) - 1);
        eap->identity_len = value_len - strlen(imsi_identity) - 1;
      }
      eap->pending_req_identity = 0;
      if (ssid == wpa_s->current_ssid) wpa_s->reassociate = 1;
      break;
    case WPA_CTRL_REQ_EAP_PASSWORD:
      bin_clear_free(eap->password, eap->password_len);
      eap->password = (u8 *)os_strdup(value);
      if (!eap->password) return -1;
    ...
    case WPA_CTRL_REQ_EAP_PASSPHRASE:
      ...
      if (!eap->cert.private_key_passwd) return -1;
    ...
    case WPA_CTRL_REQ_SIM:
      ...
      if (!eap->external_sim_resp) return -1;
      eap->pending_req_sim = 0;
      break;
    case WPA_CTRL_REQ_PSK_PASSPHRASE:
      if (wpa_config_set(ssid, "psk", value, 0) < 0) return -1;
      ssid->mem_only_psk = 1;
      if (ssid->passphrase) wpa_config_update_psk(ssid);
      if (wpa_s->wpa_state == WPA_SCANNING && !wpa_s->scanning)
        wpa_supplicant_req_scan(wpa_s, 0, 0);
      break;
    case WPA_CTRL_REQ_EXT_CERT_CHECK:
      if (eap->pending_ext_cert_check != PENDING_CHECK) return -1;
...
int wpas_network_disabled(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid) {
  ...
  for (i = 0; i < NUM_WEP_KEYS; i++) {
    ...
    if (len == 0) continue;
    if (len == 5 && (drv_enc & WPA_DRIVER_CAPA_ENC_WEP40)) continue;
    if (len == 13 && (drv_enc & WPA_DRIVER_CAPA_ENC_WEP104)) continue;
    if (len == 16 && (drv_enc & WPA_DRIVER_CAPA_ENC_WEP128)) continue;
  ...
  if (ssid->eap.imsi_privacy_cert) {
    ...
    if (failed) {
      wpa_printf(MSG_DEBUG, "Invalid imsi_privacy_cert (%s) - disable network",
...
int wpas_get_ssid_pmf(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid) {
  if (ssid == NULL || ssid->ieee80211w == MGMT_FRAME_PROTECTION_DEFAULT) {
    ...
    if (wpa_s->conf->pmf == NO_MGMT_FRAME_PROTECTION && ssid && ssid->ocv)
...
enum sae_pwe wpas_get_ssid_sae_pwe(struct wpa_supplicant *wpa_s,
                                   struct wpa_ssid *ssid) {
  if (!ssid || ssid->sae_pwe == DEFAULT_SAE_PWE) return wpa_s->conf->sae_pwe;
  return ssid->sae_pwe;
}

bool wpas_is_sae_avoided(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid,
...
int pmf_in_use(struct wpa_supplicant *wpa_s, const u8 *addr) {
  ...
  if (wpa_s->valid_links) {
    ...
    if (!ether_addr_equal(addr, wpa_s->bssid)) return 0;
  }
  return wpa_sm_pmf_enabled(wpa_s->wpa);
}

int wpas_is_p2p_prioritized(struct wpa_supplicant *wpa_s) {
  if (wpa_s->global->conc_pref == WPA_CONC_PREF_P2P) return 1;
  if (wpa_s->global->conc_pref == WPA_CONC_PREF_STA) return 0;
  return -1;
}
...
void wpas_auth_failed(struct wpa_supplicant *wpa_s, const char *reason,
  ...
  if (ssid->key_mgmt == WPA_KEY_MGMT_WPS) return;
  ...
  os_get_reltime(&now);
  if (now.sec + dur <= ssid->disabled_until.sec) return;
  ...
  int msg_len = snprintf(NULL, 0, format_str, ssid->id,
                         wpa_ssid_txt(ssid->ssid, ssid->ssid_len),
                         ssid->auth_failures, dur, reason) +
                1;
  char *msg = os_malloc(msg_len);
  if (!msg) return;
  snprintf(msg, msg_len, format_str, ssid->id,
           wpa_ssid_txt(ssid->ssid, ssid->ssid_len), ssid->auth_failures, dur,
           reason);
  wpas_notify_ssid_temp_disabled(wpa_s, msg);
  os_free(msg);

  if (bssid) os_memcpy(ssid->disabled_due_to, bssid, ETH_ALEN);
}
...
int disallowed_bssid(struct wpa_supplicant *wpa_s, const u8 *bssid) {
  ...
  if (wpa_s->disallow_aps_bssid == NULL) return 0;

  for (i = 0; i < wpa_s->disallow_aps_bssid_count; i++) {
    if (ether_addr_equal(wpa_s->disallow_aps_bssid + i * ETH_ALEN, bssid))
      return 1;
  }
...
int disallowed_ssid(struct wpa_supplicant *wpa_s, const u8 *ssid,
  ...
  if (wpa_s->disallow_aps_ssid == NULL || ssid == NULL) return 0;
...
int get_shared_radio_freqs_data(struct wpa_supplicant *wpa_s,
  ...
  dl_list_for_each(ifs, &wpa_s->radio->ifaces, struct wpa_supplicant,
    ...
    if (exclude_current && ifs == wpa_s) continue;
...
int get_shared_radio_freqs(struct wpa_supplicant *wpa_s, int *freq_array,
  ...
  num = get_shared_radio_freqs_data(wpa_s, freqs_data, len, exclude_current);
...
void wpas_vendor_elem_update(struct wpa_supplicant *wpa_s) {
  ...
  if ((wpa_s->parent == wpa_s || (wpa_s == wpa_s->p2pdev && wpa_s->p2p_mgmt)) &&
      wpa_s->global->p2p && !wpa_s->global->p2p_disabled)
...
int wpas_vendor_elem_remove(struct wpa_supplicant *wpa_s, int frame,
  ...
  for (; ie + 1 < end; ie += 2 + ie[1]) {
    if (ie + len > end) break;
    if (os_memcmp(ie, elem, len) != 0) continue;
...
struct hostapd_hw_modes *get_mode(struct hostapd_hw_modes *modes, u16 num_modes,
                                  enum hostapd_hw_mode mode, bool is_6ghz) {
  u16 i;
  ...
  for (i = 0; i < num_modes; i++) {
    if (modes[i].mode != mode || !modes[i].num_channels || !modes[i].channels)
      continue;
    if (is_6ghz == modes[i].is_6ghz) return &modes[i];
  }
...
struct hostapd_hw_modes *get_mode_with_freq(struct hostapd_hw_modes *modes,
  ...
  for (i = 0; i < num_modes; i++) {
    for (j = 0; j < modes[i].num_channels; j++) {
      if (freq == modes[i].channels[j].freq) return &modes[i];
    }
  }
...
static struct wpa_bss_tmp_disallowed *wpas_get_disallowed_bss(
    struct wpa_supplicant *wpa_s, const u8 *bssid) {
  struct wpa_bss_tmp_disallowed *bss;

  dl_list_for_each(bss, &wpa_s->bss_tmp_disallowed,
                   struct wpa_bss_tmp_disallowed, list) {
    if (ether_addr_equal(bssid, bss->bssid)) return bss;
  }
...
int wpas_enable_mac_addr_randomization(struct wpa_supplicant *wpa_s,
  ...
  if (type & MAC_ADDR_RAND_SCHED_SCAN) {
    if (wpas_mac_addr_rand_scan_set(wpa_s, MAC_ADDR_RAND_SCHED_SCAN, addr,
                                    mask))
      return -1;
...
int wpa_drv_signal_poll(struct wpa_supplicant *wpa_s,
  ...
  if (!wpa_s->driver->signal_poll) return -1;
  ...
  if (res == 0) {
    ...
    dl_list_for_each(dso, &wpa_s->drv_signal_override,
                     struct driver_signal_override, list) {
      if (!ether_addr_equal(wpa_s->bssid, dso->bssid)) continue;
      wpa_printf(
          MSG_DEBUG,
          "Override driver signal_poll information: current_signal: %d->%d "
          "avg_signal: %d->%d avg_beacon_signal: %d->%d current_noise: %d->%d",
          si->data.signal, dso->si_current_signal, si->data.avg_signal,
          dso->si_avg_signal, si->data.avg_beacon_signal,
          dso->si_avg_beacon_signal, si->current_noise, dso->si_current_noise);
...
struct wpa_scan_results *wpa_drv_get_scan_results(struct wpa_supplicant *wpa_s,
                                                  const u8 *bssid) {
  struct wpa_scan_results *scan_res;
#ifdef CONFIG_TESTING_OPTIONS
  size_t idx;
  ...
  if (wpa_s->driver->get_scan_results)
    scan_res = wpa_s->driver->get_scan_results(wpa_s->drv_priv, bssid);
  ...
  for (idx = 0; scan_res && idx < scan_res->num; idx++) {
    ...
    dl_list_for_each(dso, &wpa_s->drv_signal_override,
                     struct driver_signal_override, list) {
      if (!ether_addr_equal(res->bssid, dso->bssid)) continue;
      wpa_printf(MSG_DEBUG,
                 "Override driver scan signal level %d->%d for " MACSTR,
                 res->level, dso->scan_level, MAC2STR(res->bssid));
...
bool wpas_ap_link_address(struct wpa_supplicant *wpa_s, const u8 *addr) {
  ...
  if (!wpa_s->valid_links) return false;

  for_each_link(wpa_s->valid_links, i) {
    if (ether_addr_equal(wpa_s->links[i].bssid, addr)) return true;
  }
...
int wpa_drv_send_action(struct wpa_supplicant *wpa_s, unsigned int freq,
                        ...
                        int no_cck) {
  if (!wpa_s->driver->send_action) return -1;

  if (data_len > 0 && data[0] != WLAN_ACTION_PUBLIC) {
    if (wpas_ap_link_address(wpa_s, dst)) dst = wpa_s->ap_mld_addr;
  ...
  for (i = 0; i < wpa_s->hw.num_modes; i++) {
    if (wpa_s->hw.modes[i].mode == HOSTAPD_MODE_IEEE80211A) {
      ...
      for (j = 0; j < wpa_s->hw.modes[i].num_channels; j++) {
        if (only_enabled && (chnl[j].flag & HOSTAPD_CHAN_DISABLED)) continue;
        if (is_6ghz_freq(chnl[j].freq)) return true;
      }
    }
  }
...
bool wpas_ap_supports_rsn_overriding(struct wpa_supplicant *wpa_s,
  ...
  for (i = 0; i < MAX_NUM_MLD_LINKS; i++) {
    if (!(wpa_s->valid_links & BIT(i))) continue;
...
bool wpas_ap_supports_rsn_overriding_2(struct wpa_supplicant *wpa_s,
  ...
  if (!bss) return false;
  if (wpa_bss_get_vendor_ie(bss, RSNE_OVERRIDE_2_IE_VENDOR_TYPE)) return true;
...
int wpas_get_owe_trans_network(const u8 *owe_ie, const u8 **bssid,
  ...
  if (end - pos < ETH_ALEN + 1) return -1;
  *bssid = pos;
  pos += ETH_ALEN;
  ssid_len_tmp = *pos++;
  if (end - pos < ssid_len_tmp || ssid_len_tmp > SSID_MAX_LEN) return -1;
